...
function ProductGrid({ products, loading }) {
  if (loading) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="bg-[#1E293B] rounded-xl h-56 md:h-64 animate-pulse"></div>
        ))}
      </div>
    )
  }
  ...
  if (!products || products.length === 0) {
    return (
      <div className="bg-[#1E293B] rounded-xl p-8 md:p-12 text-center border border-[#334155]">
